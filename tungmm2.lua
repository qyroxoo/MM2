...
for _, g in ipairs(mountTarget:GetChildren()) do
    if g.Name == "Obsidian" or g.Name == "MONO_ESP" or g.Name == "MONO_MINI" then pcall(function() g:Destroy() end) end
end
local TRACKED = {}
...
do
    ...
    local t0=os.clock()
    while left>0 and os.clock()-t0<20 do task.wait() end
    for i,p in ipairs(paths) do
        if not srcs[i] then srcs[i]=game:HttpGet(repo..p) end
    end
end
...
local flags={autoKill=false,knifeWalls=false,gunWalls=false,instantKnife=false,gunEsp=false,gunEspDist=false,autoGun=false,aimbot=false,silentAim=false,triggerBot=false,showFov=false,
    fly=false,flySpeed=60,noclip=false,infJump=false,unlockCam=false,
    espBox=false,espChams=false,espFill=false,espNames=false,espRoleTags=false,espSkeleton=false,killFeed=false,coinEsp=false,
    fullbright=false,fpsBoost=false,autoCoins=false,murdererNotify=false,antiAfk=false,
    miniSquare=false,showBindNote=false}
...
if game.GameId~=MM2_UNIVERSE then
    ...
    pcall(function() if GENV then GENV.MONO_Unload=function() pcall(function() Library:Unload() end) end end end)
    return
end
...
local function playerHasTagged(plr,tag)
    for _,t in ipairs(CollectionService:GetTagged(tag)) do
        local par=t.Parent
        if par then
            if par==plr.Character then return true end
            if par:IsA("Backpack") and par.Parent==plr then return true end
        end
    end
    return false
end
...
local function isEnemyOf(myrole,role)
    if myrole=="Murderer" then return isGunRole(role) end
...
local function aimPos(plr,noPredict)
    local ch=plr and plr.Character; local hrp=ch and getHRP(ch); if not hrp then return end
    local base=(ch:FindFirstChild("UpperTorso") or ch:FindFirstChild("Torso") or hrp).Position
    if noPredict then return base end
    local vel=hrp.AssemblyLinearVelocity
    if vel.Magnitude<PREDICT_MIN_SPEED then return base end
    return base+vel*netPing()
end
...
local function aimAt(target,role,throughWalls)
    ...
    elseif isGunRole(role) then
        shootGunAt(aimPos(target),throughWalls==true); return 0.15
    end
    return 0
end
...
local function fovTarget()
    ...
    for _,p in ipairs(Players:GetPlayers()) do if p~=LocalPlayer and alive(p) and isEnemyOf(mr,roleOf(p)) then
  ...
  local okLoop,errLoop=pcall(function()
    if flags.autoKill then
        ...
        if role=="Murderer" then
            ...
            task.wait(0.1)
        elseif isGunRole(role) then
            local tgt=nearest(list); if tgt and flags.autoKill then shootGunAt(aimPos(tgt),true) end
            task.wait(0.12)
...
task.spawn(function() local last=0 while not Library.Unloaded do
    if flags.triggerBot then local role=myRole()
        if isGunRole(role) then local t=fovTarget()
            local walls=flags.gunWalls
            if t and os.clock()-last>0.3 and (walls or hasLineOfSight(t.Character)) then
                last=os.clock(); aimAt(t,role,walls)
            end
        end
    end
    task.wait(0.05)
            ...
            if hrp then
                ...
                if sp.Z>0 then
                    ...
                    if d<=radius and (not bd or d<bd) then
                        bd=d
                        best=(ch:FindFirstChild("UpperTorso") or ch:FindFirstChild("Torso") or hrp).Position
                    end
                end
            end
        end
...
local function computeSilentTarget()
    local mr=myRole()
    if mr=="Murderer" then return crosshairAnyPlayerPos(aimFov) end
    if isGunRole(mr) then return crosshairEnemyPos(aimFov) end
    return nil
end
...
local function refreshGuiRects()
    ...
    local function add(o)
        if o and o.Visible and o.AbsoluteSize.X>1 and o.AbsoluteSize.Y>1 then
            local p,s=o.AbsolutePosition,o.AbsoluteSize
            guiRects[#guiRects+1]={p.X,p.Y,p.X+s.X,p.Y+s.Y}
        end
    end
    if Library.Toggled then add(sg:FindFirstChild("Main")) end
    for _,c in ipairs(sg:GetChildren()) do
        if c:IsA("GuiObject") and c.Name~="Main" then add(c) end
    end
    if MiniBtnRef and MiniBtnRef.Visible then add(MiniBtnRef) end
end
...
local function ensureBox(plr)
    ...
    b={}
    for i=1,4 do
        local l=Drawing.new("Line"); l.Thickness=1; l.Transparency=1; l.Visible=false; b[i]=l
    end
...
local function charScreenRect(ch)
    ...
    for i=0,7 do
        local ox=(i%2==0) and -hx or hx
        local oy=(math.floor(i/2)%2==0) and -hy or hy
        local oz=(math.floor(i/4)%2==0) and -hz or hz
        local w=(cf*CFrame.new(ox,oy,oz)).Position
        local v=Camera:WorldToViewportPoint(w)
        if v.Z>0 then
            anyFront=true
...
bind(RunService.RenderStepped,function()
    if not flags.espBox or typeof(Drawing)~="table" then
        if next(boxStore) then for p in pairs(boxStore) do clearBox(p) end end
        return
    end
    refreshGuiRects()
    for _,plr in ipairs(Players:GetPlayers()) do if plr~=LocalPlayer then
...
local function ensureSkel(plr,ch)
    ...
    for i=1,#bones do
        local l=Drawing.new("Line"); l.Thickness=2; l.Transparency=1; l.Visible=false; s.lines[i]=l
...
bind(RunService.RenderStepped,function()
    ...
    if not flags.espBox then refreshGuiRects() end
    for _,plr in ipairs(Players:GetPlayers()) do if plr~=LocalPlayer then
        local ch=plr.Character
        if ch and ch:FindFirstChildOfClass("Humanoid") and alive(plr) then
...
task.spawn(function()
    ...
    while not Library.Unloaded do
        if flags.autoCoins and not coinBagFull then
            ...
            if hrp and hum then
                ...
                for _,c in ipairs(coinCache) do
                    if c.Parent and not coinTaken(c) and not (farmBlack[c] and os.clock()<farmBlack[c]) then
                        ...
                        if not bd or d<bd then bd,coin=d,c end
                    end
                end
                if coin then
...
local droppedGun
task.spawn(function() while not Library.Unloaded do
    droppedGun=(flags.gunEsp or flags.autoGun) and findDroppedGun() or nil
    task.wait(0.3)
end end)
...
local function grabGunOnce()
    ...
    local h=droppedGun or findDroppedGun()
    local hrp=getHRP(LocalPlayer.Character)
    if not (h and h.Parent and hrp and alive(LocalPlayer)) then return false end
    grabbing=true
...
task.spawn(function() while not Library.Unloaded do
    if flags.autoGun and not grabbing and droppedGun and myRole()~="Murderer" and not findWeapon("Gun") then
        grabGunOnce()
    end
    task.wait(0.3)
end end)
...
local function flingPlayer(p)
    ...
    if not (myHrp and hum and tHrp) then notify("Can't fling — you or the target has no character"); return end
    flinging=true
    task.spawn(function()
        local back=myHrp.CFrame
        local restore={}
        for _,pp in ipairs(myCh:GetDescendants()) do
            if pp:IsA("BasePart") then restore[pp]=pp.CanCollide; pp.CanCollide=true end
        end
        pcall(function() hum.PlatformStand=true end)
        local t0=os.clock()
        while os.clock()-t0<FLING_SECONDS do
            local h=getHRP(LocalPlayer.Character); local t=p.Character and getHRP(p.Character)
            if not (h and t) then break end
            h.CFrame=t.CFrame
            h.AssemblyLinearVelocity=Vector3.zero
            h.AssemblyAngularVelocity=Vector3.new(0,9e4,0)
            RunService.Heartbeat:Wait()
        end
        for _=1,14 do
        ...
        for pp,cc in pairs(restore) do if pp and pp.Parent then pp.CanCollide=cc end end
        ...
        for _=1,10 do
            ...
            if (h3.Position-back.Position).Magnitude>60 then
                h3.CFrame=back
...
do
    ...
    task.spawn(function() while not Library.Unloaded do scanDeaths(); task.wait(0.5) end end)
end
...
do local g=Tabs.Combat:AddLeftGroupbox("Auto")
    g:AddToggle("AutoKill",{Text="Auto Kill",Tooltip="Murderer kills everyone at once. Sheriff or Hero shoots the murderer.",Callback=function(v) flags.autoKill=v end})
...
do local g=Tabs.Combat:AddLeftGroupbox("Sheriff Gun")
    ...
    g:AddToggle("AutoGun",{Text="Auto Grab Gun",Tooltip="Grabs the dropped gun the moment it appears, then returns you.",Callback=function(v) flags.autoGun=v end})
...
do local g=Tabs.Player:AddLeftGroupbox("Movement")
    g:AddToggle("Fly",{Text="Fly",Tooltip="WASD to move, Space up, Ctrl down. Relative to your camera.",Callback=function(v) flags.fly=v; if v then startFly() else stopFly() end end})
    ...
    g:AddToggle("Noclip",{Text="Noclip",Tooltip="Walk through walls. Pauses automatically during a fling.",Callback=function(v) flags.noclip=v end})
...
do local g=Tabs.Teleport:AddLeftGroupbox("Player Actions")
    g:AddDropdown("TpPlayer",{SpecialType="Player",ExcludeLocalPlayer=true,Text="Select Player",Tooltip="Target used by both buttons below"})
...
do local g=Tabs.Teleport:AddRightGroupbox("Coins")
    g:AddToggle("AutoCoins",{Text="Auto Collect Coins",Tooltip="Flies you to coins at normal walkspeed. Stops on its own when your bag is full.",Callback=function(v) flags.autoCoins=v; if v then if coinBagFull then notify("Coin bag is already full") else notify("Auto-collect on") end end end})
    g:AddButton({Text="Teleport To Nearest Coin",Tooltip="One hop to the closest uncollected coin",Func=function() local hrp=getHRP(LocalPlayer.Character); if not hrp then return end
...
MenuGroup:AddToggle("MiniSquare",{Text="Minimize To Square",Tooltip="Shows a small draggable square while the menu is hidden. Click it to reopen.",Default=false,Callback=function(v) flags.miniSquare=v end})
...
do
    ...
    g:AddButton({Text="Save As Default",Tooltip="Loads this theme automatically next time",Func=function()
        ...
        notify(ok and ("Default theme: "..tostring(Options.ThemePick.Value)) or "Couldn't save default theme")
    end})
    g:AddButton({Text="Clear Default",Tooltip="Goes back to the stock theme on next launch",Func=function()
        pcall(function() ThemeManager:DeleteDefaultTheme() end)
        notify("Default theme cleared")
    end})
...
Library:OnUnload(function()
    pcall(function() if GENV then GENV.MONO_Unload=nil; GENV.MONO_GUIS=nil end end)
